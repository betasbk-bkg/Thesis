...
Output: paper_fullscale_results.json (development-era full-scale runner; the paper archives are data/*.json — see README)
========================================================================
"""
import numpy as np
import time
...
def gen_votes(ideal_angle, prev_angle, troll_ratio, N_agents, rng):
    ...
    Agent composition (subtype counts are integer-rounded, so realized ratios differ from nominal at small N):
      accurate: 70/95 (~73.7%) of the non-adversarial population, +/-3 deg noise
      slow:     20/95 (~21.1%) of the non-adversarial population, lagged direction
...
if __name__ == '__main__':
    # This file is the simulation ENGINE.  Running it directly used to launch a
    # development-era campaign (MC = 10 for parts of it), which is not the
    # configuration reported in the manuscript.  That runner is kept as
    # legacy/development_full_runner.py for provenance.
    print(__doc__ or '')
    print("simulation_main.py provides the simulation engine only.")
    print("For manuscript reproduction run, from the repository root:")
    print("    python reproduce_all.py")
    print("or see README.md for the individual commands.")
    print("The superseded development campaign is legacy/development_full_runner.py.")
