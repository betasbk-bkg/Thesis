...
SIM-1  system delay tau      6 levels x 3 adversarial ratios x 10 speeds   8,100 runs
...
SIM-4  disruptor behaviour   uniform / coherent-opposite / lagged          1,080 runs
...
# ------------------------------------------------------------------
def validation_gate():
    print("=" * 60, "\n[GATE] method_comparison spot reproduction check")
    ...
    for traj_name, tr, m, v in checks:
        ...
        key = key.replace('_v5.0_', '_v5.0_')  # key format unchanged
        if key not in ref:
            key = f"{traj_name}_tr{tr}_v{v}_{m}"
        stored = ref.get(key)
        if stored is None:
            print(f"  [warn] key not found: {key} - check manually"); continue
        r = [sm.simulate(sm.Circle(), N_FIXED, tr, seed=i * SEED_BASE,
                         method=m, speed_override=v)['rmse'] for i in range(MC)]
        mine, theirs = np.mean(r), stored['rmse_mean']
        tol = 3 * stored['rmse_std'] / np.sqrt(MC) + 1e-4
        flag = "OK" if abs(mine - theirs) <= max(tol, 0.01) else "MISMATCH"
        if flag != "OK": ok = False
        print(f"  {key}: reproduced {mine:.4f} vs archived {theirs:.4f} (tol {max(tol,0.01):.4f}) [{flag}]")
    if not ok:
        print("  [note] seed-offset differences are possible; only physically large mismatches warrant stopping.")
    print("=" * 60)

# ------------------------------------------------------------------
# SIM-1: system delay tau sensitivity
# ------------------------------------------------------------------
def sim1():
...
# ------------------------------------------------------------------
# SIM-4: disruptor behaviour variants
# ------------------------------------------------------------------
