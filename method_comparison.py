...
Claim 1 근거: "같은 속도에서 aggregation method 간 차이 없다"

Engine: paper_fullscale.py 동일 (ceiling_verify.py compatible)
MC=15 (논문 전체 통일)

실험 설계:
  조작 변수: method (fixed, majority, quadratic)
            speed (1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0)
  통제 변수: N=150 고정, agent 비율 고정
  환경 변수: troll (0.05, 0.20, 0.40), trajectory (circle, square)
  측정 변수: RMSE, gamma, actual speed

  Total: 3 × 8 × 3 × 2 × 15 = 2,160 sims
  Estimated: ~20 min

통계 검정:
  1) Welch's t-test: method 쌍별 (Fixed vs Majority 등)
  2) One-way ANOVA: 3 method 동시 비교
  3) Effect size (Cohen's d): 실질적 차이 크기
...
# ====================================================================
# ENGINE (paper_fullscale.py 동일 — 복사)
# ====================================================================
DT        = 1/60
MSPD      = 5.0
SMOOTH    = 0.2
...
def main():
        ...
        for tr in trolls:
            for v in speeds:
                for m in methods:
                    rmses = []
                    for i in range(MC):
                        seed = i * 31 + int(v * 100) + hash(m) % 10000
    ...
    print(f"  H0: 같은 속도에서 두 method의 RMSE 평균이 같다")
    ...
    outfile = 'method_comparison_results.json'
