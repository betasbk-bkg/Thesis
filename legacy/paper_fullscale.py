"""
========================================================================
Full-scale simulation (legacy characterization run)
"Speed Over Strategy: Performance Determinants in
 Adversarial Crowd-Sourced Shared Control"

Engine: ceiling_verify.py compatible (validated against existing data)
========================================================================

Experiments:
  E2e: N sweep × new troll ratios (10%, 15%, 30%) — circle, square
  E2f: N sweep × all trolls — lemniscate, zigzag
  E1e: Speed sweep × all trolls — circle, square
  E3c: Cross-validation (multiplicative model)
  E3d: New condition prediction

Total: ~4,200 sims, estimated 40-50 min

Usage: python3 paper_fullscale.py
Output: paper_fullscale_results.json (all raw data + analysis)
========================================================================
"""
import numpy as np
import time
...
# ====================================================================
# SECTION 0: ENGINE CONSTANTS (Unity-matched, DO NOT MODIFY)
# ====================================================================
DT        = 1/60       # 60 fps
MSPD      = 5.0        # max speed (m/s)
SMOOTH    = 0.2        # Lerp smoothing factor
WIN       = 0.3        # vote window (sec)
DELAY_F   = 26         # input delay (frames) = 433ms
DUR       = 65.0       # simulation duration (sec)
FRAMES    = int(DUR / DT)  # total frames = 3900
LOOK      = 2.0        # look-ahead distance (m)
