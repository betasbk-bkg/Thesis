...
axes[1].text(0.05,0.86,"$v^*\\leq1.0$ m/s (monotonic)",transform=axes[1].transAxes,fontsize=10,
...
ax.set_xlabel("State-observation delay $\\tau$ (ms)"); ax.set_ylabel("$\\eta^2_{\\mathrm{speed}}\\,/\\,\\eta^2_{\\mathrm{method}}$")
...
ax.set_xlabel("Nominal agent speed $v$ (m/s)"); ax.set_ylabel("RMSE (m)")
ax.set_title("$\\bf{(b)}$ Speed\u2013error curves at three delays",loc="left")
