...
ax.set_title("(a) Nominal-speed axis",loc="left"); ax.legend(frameon=False,fontsize=9)
...
ax.set_ylabel("RMSE (m)"); ax.set_title("(b) Effective-speed axis",loc="left")
...
for ax,traj,title in [(axes[0],"circle","(a) Circle trajectory"),(axes[1],"square","(b) Square trajectory")]:
...
ax.set_xlabel("Number of agents $N$"); ax.set_ylabel("RMSE (m)")
...
ax.set_xlabel("System delay $\\tau$ (ms)"); ax.set_ylabel("$\\eta^2_{speed}\\,/\\,\\eta^2_{method}$")
ax.set_title("(a) Dominance ratio across delay",loc="left"); ax.legend(frameon=False,fontsize=9); ax.grid(alpha=0.25,which="both")
ax=axes[1]
...
ax.set_xlabel("Nominal agent speed $V$ (m/s)"); ax.set_ylabel("RMSE (m)")
ax.set_title("(b) Speed leverage collapses as delay vanishes",loc="left")
