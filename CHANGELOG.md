...
Figure corrections following a pre-submission audit of the deposited figures. No data file,
no analysis result and no reported statistic changes; only figure rendering and the labels
in the manuscript that had to follow.

- `analysis/regenerate_figures.py`
  - Figures 1, 3 and 6b: the speed axis is now labelled `v`, matching the convention declared
    in the Methods ("`V` ... written `v` in the Results, figures, and model sections"). The
    consensus-scaled gain law is written `gamma^2 v` for the same reason, in the figures and in
    the manuscript text.
  - Figure 3b: the `v* <= 1.0 m/s (monotonic)` annotation was placed at the lower-left corner of
    the axes, where it overlapped the six curves at v = 1.0 and 1.5 m/s. It is moved to the empty
    upper-left region; no data point is now obscured.
  - Figure 4: the adversarial ratio in the legend is set as the italic variable `p`, matching the
    italic `N` on the same axis and the manuscript.
  - Figure 6a: the axis label subscripts are set in Roman (`eta^2_speed / eta^2_method`), matching
    the manuscript, Table 1/2 and Supplementary Tables S6/S11. Nature Portfolio style sets
    subscripts that are labels in Roman.
  - Figure 6b: the panel title read "Ratio versus RMSE span" while the panel shows RMSE(v) curves
    at three delays; it now describes what the panel plots.
- `analysis/model_refit120.py` - Figure 5 legend: sample size set as the italic variable `n`.
- Figures 2 and S1 are unchanged (Figure 2 regenerates pixel-identically; only its metadata differs,
  so the deposited file is retained).
